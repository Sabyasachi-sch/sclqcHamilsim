...
def XX_Block_diag(input_str, q_no):
    ...
    if input_str[:-1] == 'I' * (len(input_str) - 1) and input_str[-1] == 'Z':
...
def generate_kron(input_str, use_hadamard=False):
    if use_hadamard:
        matrix_map = {
            'I': I_sp,
            'Z': had_mat_sp,  # Use Hadamard matrix at 'Z' position
        }
    else:
        matrix_map = {
            'I': I_sp,
            'Z': Z_sp,  # Use Pauli Z for 'Z'
        }
...
def ckt_diag(input_str, q_no, argm, time_step):
    ...
    Hamil_conv = generate_kron(input_str, use_hadamard=True) 
    ...
    resultant_mat1 = spcsc(Hamil_conv.dot(resultant_mat0.dot(Hamil_conv)))   
...
##### Example: To determine the matrix corresponding to the Scalable Circuit model for a Pauli-Z String

##### ckt_mat returns the circuit matrix in sparse format and error returns the Frobenius norm.

ckt_mat, error = ckt_diag("ZZIZIZ", 6, 0.8, 0.001)
