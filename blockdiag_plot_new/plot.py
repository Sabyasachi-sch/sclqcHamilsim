...
plt.ylim(1.e-19,2.e-13)

plt.grid(color='grey', which='both', axis='both', alpha=0.7)

fig.savefig('bdiag.png', dpi=100)

plt.show()

############################################################################

tick_list = np.logspace(np.log10(min(Frb_norm3)), np.log10(max(Frb_norm18)), num=8)
...
ax.text(.01, 1.06, '(b)',color="black", fontsize=25, ha='left', va='top', transform=plt.gca().transAxes)

fig.savefig('blockdiag.png', dpi=300)
